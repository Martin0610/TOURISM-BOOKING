              ...
              {items.map(item => (
                <div key={item.id} className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm overflow-hidden flex flex-col">
                  <div className="h-40 bg-gradient-to-br from-blue-400 to-indigo-500 relative">
                    ...
                    <span className="absolute top-3 left-3 bg-white/90 text-blue-700 text-xs font-semibold px-2 py-0.5 rounded-full">{item.package.category}</span>
                  </div>
                  <div className="p-4 flex-1 flex flex-col justify-between">
                    ...
                    <div className="flex items-center justify-between mt-4">
                      <span className="text-blue-600 font-bold text-lg">₹{item.package.pricePerPerson.toLocaleString('en-IN')}<span className="text-xs font-normal text-gray-400">/person</span></span>
                      <div className="flex gap-2">
                        <Link href={`/packages/${item.package.id}`} className="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-sm hover:bg-blue-700 transition">Book</Link>
